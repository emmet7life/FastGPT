...
import { uploadFileAsync } from '@/web/core/dataset/hengda/api';
...
const Upload = () => {
  ...
  const { mutate: startUpload, isLoading } = useRequest({
    mutationFn: async (importForm: ImportFormType) => {
      if (sources.length === 0) return;
      const filterWaitingSources = sources.filter((item) => item.createStatus === 'waiting' && item.file);
      ...
      for await (const item of filterWaitingSources) {
        ...
        if (importSource === ImportDataSourceEnum.fileUpload && item.file) {
