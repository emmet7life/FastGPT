...
import {
  updateCollectionChunkById,
  postInsertData2DatasetV2
...
const InputDataModal = ({
  ...
  fileName?: string;
  filePath?: string;
  defaultValue?: { q: string; a?: string, state?: number /** 文档嵌入完成后，更新文档时必须传1 */ };
  onClose: () => void;
  onSuccess: (data: InputDataType & { dataId: string, content: string }) => void;
}) => {
  ...
  const { mutate: sureImportData, isLoading: isImporting } = useRequest({
    mutationFn: async (e: InputDataType) => {
      ...
      if (!fileName || !filePath) {
        setCurrentTab(TabEnum.content);
        return Promise.reject(t('common:common.Error'));
      }
      ...
      const dataId = await postInsertData2DatasetV2({
  ...
  const { runAsync: onUpdateData, loading: isUpdating } = useRequest2(
    async (e: InputDataType) => {
      ...
      // not exactly same
      await updateCollectionChunkById({
        id: dataId,
        document: e.q,
        state: defaultValue?.state || 0,
        // ...e,
        // indexes:
        //   e.indexes?.map((index) =>
  ...
  // const isLoading = false;//isFetchingData;
