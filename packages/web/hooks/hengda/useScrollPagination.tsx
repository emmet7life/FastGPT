...
import { PaginationProps, PaginationResponse } from '@fastgpt/global/core/hengda/dataset/type';
import {
  useBoolean,
  useLockFn,
  useMemoizedFn,
...
>(
  api: (data: TParams) => Promise<TData>,
  {
    ...
    itemHeight: number | ItemHeight<TData['documents'][0]>;
    overscan?: number;
  ...
  const [data, setData] = useState<TData['documents']>([]);
  ...
  const [list] = useVirtualList<TData['documents'][0]>(data, {
  ...
  const loadData = useLockFn(async (init = false) => {
    ...
    const pageNum = init ? 1 : pageNumber;
...
>(
  ...
  const loadData = useLockFn(
    async (init = false, ScrollContainerRef?: RefObject<HTMLDivElement>) => {
      ...
      const pageNumber = init ? 1 : pageNum;
      ...
      try {
        ...
        if (scrollLoadType === 'top') {
          ...
          setData((prevData) => (pageNumber === 1 ? res.documents : [...res.documents, ...prevData]));
          setPageNum(pageNumber + 1);
          adjustScrollPosition();
        } else {
          setData((prevData) => (pageNumber === 1 ? res.documents : [...prevData, ...res.documents]));
          setPageNum(pageNumber + 1);
        }
      } catch (error: any) {
        if (showErrorToast) {
