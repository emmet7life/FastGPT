...
import type { DocumentPagingData } from '@fastgpt/global/core/hengda/dataset/type';
...
export const convertArrayToCamelCase = (array: any[]): any[] => {
  return array.map((item) => convertToCamelCase(item));
};
...
export function usePagination<ResT = any>({
  ...
  api: (data: any) => Promise<DocumentPagingData<ResT>>;
  pageSize?: number;
  ...
  const fetchData = useLockFn(
    async (num: number = pageNum, ScrollContainerRef?: RefObject<HTMLDivElement>) => {
      ...
      try {
        const res: DocumentPagingData<ResT> = await api({
          pageNum: num,
          pageSize,
          ...params
        });
        ...
        // Check total and set
        setPageNum(num);
        res.total_records !== undefined && setTotal(res.total_records);

        let dataList = [];
        res.total_records !== undefined && setTotal(res.total_records);
        if (res.documents && res.documents.length > 0) {
          const convertedData = convertArrayToCamelCase(res.documents); // 转换数据
          dataList = convertedData as ResT[];
        } else {
          dataList = res.documents || [];
        }
        ...
        if (type === 'scroll') {
          if (scrollLoadType === 'top') {
            ...
            setData((prevData) => (num === 1 ? dataList : [...dataList, ...prevData]));
            adjustScrollPosition();
          } else {
            setData((prevData) => (num === 1 ? dataList : [...prevData, ...dataList]));
          }
        } else {
          setData(dataList);
        }
